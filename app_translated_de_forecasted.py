...
def main():
    ...
    with col2:
        ...
        <div class="metric-card">
            ...
            <p style="margin:0; color: #666;">Kritische Exits</p>
        </div>
        """, unsafe_allow_html=True)
...
# --- FORECASTING SECTION ---
st.markdown("---")
st.markdown('<h3 class="section-header">🔮 Ressourcenprognose & Komponentenrisiko</h3>', unsafe_allow_html=True)

# Forecast next 12 months
future_months = pd.date_range(datetime.today(), periods=12, freq='MS')
forecast_df = pd.DataFrame({'month': future_months})
forecast_df['active_members'] = forecast_df['month'].apply(lambda m: df[(df['start_date'] <= m) & (df['planned_exit'] >= m)].shape[0])
forecast_df['exits'] = forecast_df['month'].apply(lambda m: df[(df['planned_exit'].dt.month == m.month) & (df['planned_exit'].dt.year == m.year)].shape[0])

# Line chart for active members
fig_forecast = go.Figure()
fig_forecast.add_trace(go.Scatter(x=forecast_df['month'], y=forecast_df['active_members'], mode='lines+markers', name='Aktive Mitglieder'))
fig_forecast.add_trace(go.Bar(x=forecast_df['month'], y=forecast_df['exits'], name='Austritte', marker_color='red'))
fig_forecast.update_layout(title='📈 Teamprognose (nächste 12 Monate)', xaxis_title='Monat', yaxis_title='Anzahl Personen', height=400)
st.plotly_chart(fig_forecast, use_container_width=True)

# --- COMPONENT RISK ANALYSIS ---
st.markdown("---")
st.markdown('<h3 class="section-header">🧩 Komponentenrisikoanalyse</h3>', unsafe_allow_html=True)
component_input = st.text_input("🔍 Komponente eingeben (z.B. Payment API)")
months_ahead = st.slider("Zeitraum in Monaten", 1, 24, 6)

if component_input:
    end_date = datetime.today() + pd.DateOffset(months=months_ahead)
    at_risk = df[(df['components'].str.contains(component_input, case=False)) & (df['planned_exit'] <= end_date)]
    if not at_risk.empty:
        st.warning(f"⚠️ Die Komponente '{component_input}' ist in den nächsten {months_ahead} Monaten gefährdet:")
        st.dataframe(at_risk[['name', 'role', 'planned_exit', 'knowledge_transfer_status', 'priority']])
    else:
        st.success(f"✅ Kein Risiko für '{component_input}' in den nächsten {months_ahead} Monaten.")
