...
# CUSTOM CSS WITH TURQUOISE BLUE CRITICAL THEME
st.markdown("""
<style>
    .main-header {
...
def main():
    ...
    with col1:
        ...
        <div class="metric-card">
            ...
            <p style="margin:0; color: #666;">Total Team Members</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        <div class="metric-card">
            ...
            <p style="margin:0; color: #666;">Critical Cases</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col3:
        ...
        <div class="metric-card">
            ...
            <p style="margin:0; color: #666;">Knowledge Transfer Complete</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col4:
        ...
        <div class="metric-card">
            ...
            <p style="margin:0; color: #666;">Avg Team Tenure</p>
        </div>
        """, unsafe_allow_html=True)
    
    # CRITICAL ALERTS SECTION
    st.markdown("---")
    st.markdown('<h3 class="section-header">🔷 Critical Resource Alerts</h3>', unsafe_allow_html=True)
                ...
                <div class="critical-alert">
                    <div style="display: flex; justify-content: space-between; align-items: start;">
                        <div>
                            ...
                            <p style="margin:0.3rem 0;">Knowledge Transfer: <b>{person['knowledge_transfer_status']}</b> | Priority: <b>{person['priority']}</b></p>
                        </div>
                    ...
                    <p style="margin:0.5rem 0 0 0; font-style: italic;">💡 Recommended action: Start hiring process immediately</p>
                </div>
                """, unsafe_allow_html=True)
        else:
            st.success("✅ No critical staffing gaps in the next 6 months")
    else:
        st.info("ℹ️ No team data available. Add team members to see critical alerts.")
    
    # EDIT/DELETE INTERFACE
    st.markdown("---")
    st.markdown('<h3 class="section-header">✏️ Manage Team Members</h3>', unsafe_allow_html=True)
        ...
        for i, member in enumerate(st.session_state.team_data):
            with st.expander(f"👤 {member['name']} - {member['role']}", expanded=False):
                ...
                with col1:
                    ...
                    st.write(f"**Start Date:** {member['start_date']}")
                    st.write(f"**Planned Exit:** {member['planned_exit']}")
                    st.write(f"**Knowledge Transfer:** {member['knowledge_transfer_status']}")
                    st.write(f"**Priority:** {member['priority']}")
    ...
    if st.session_state.editing_index is not None:
        st.markdown("---")
        st.markdown('<h3 class="section-header">📝 Edit Team Member</h3>', unsafe_allow_html=True)
        ...
        with st.form(f"edit_form_{edit_index}"):
            ...
            with col1:
                edit_name = st.text_input("Full Name", value=member['name'])
                edit_role = st.text_input("Role/Position", value=member['role'])
                edit_components = st.text_area("Key Components/Responsibilities", value=member['components'])
            
            with col2:
                edit_start_date = st.date_input("Start Date", value=datetime.strptime(member['start_date'], "%Y-%m-%d"))
                edit_planned_exit = st.date_input("Planned Exit Date", value=datetime.strptime(member['planned_exit'], "%Y-%m-%d"))
                edit_status = st.selectbox("Knowledge Transfer Status", 
                                         ["Not Started", "In Progress", "Completed"],
                                         index=["Not Started", "In Progress", "Completed"].index(member['knowledge_transfer_status']))
                edit_priority = st.selectbox("Priority Level",
    ...
    if not df.empty:
        st.markdown("---")
        st.markdown('<h3 class="section-header">📈 Strategic Overview</h3>', unsafe_allow_html=True)
        ...
        with col1:
            ...
            fig_timeline = px.timeline(df, x_start="start_date", x_end="planned_exit", y="name",
                                     color="priority", 
                                     title="Team Member Timeline (Color by Priority)",
                                     color_discrete_map={
                                         "Critical": "#00bcd4",
                                         "High": "#009999", 
                                         "Medium": "#4dd0e1",
        ...
        with col2:
            ...
            fig_donut = px.pie(values=status_counts.values, names=status_counts.index, 
                              title="Knowledge Transfer Status Overview",
                              hole=0.4,
    ...
    if not df.empty:
        st.markdown("---")
        st.markdown('<h3 class="section-header">👥 Detailed Team Overview</h3>', unsafe_allow_html=True)
        
        # Filters
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            status_filter = st.multiselect("Knowledge Transfer", 
                                         options=df['knowledge_transfer_status'].unique(),
                                         default=df['knowledge_transfer_status'].unique())
        with col2:
            priority_filter = st.multiselect("Priority Level",
        ...
        with col4:
            days_filter = st.slider("Days Until Exit", 
                                  min_value=0, 
        ...
        display_df.columns = ['Name', 'Role', 'Components', 'Priority', 'Days Until Exit', 'KT Status']
        
        # Color code the Days Until Exit column
        def color_days(val):
            if val < 90:
                color = '#00bcd4'
            elif val < 180:
        ...
        styled_df = display_df.style.applymap(color_days, subset=['Days Until Exit'])
        st.dataframe(styled_df, use_container_width=True)
    
    # ADD NEW MEMBER FORM IN SIDEBAR
    st.sidebar.markdown('<h3 style="color: #009999;">➕ Add Team Member</h3>', unsafe_allow_html=True)
    
    with st.sidebar.form("add_member", clear_on_submit=True):
        name = st.text_input("Full Name")
        role = st.text_input("Role/Position")
        components = st.text_area("Key Components/Responsibilities")
        ...
        with col3:
            status = st.selectbox("Knowledge Transfer", ["Not Started", "In Progress", "Completed"])
        with col4:
        ...
        if submitted:
            if name and role:
                ...
                st.sidebar.error("Please fill at least Name and Role")
    
    # SIDEBAR ACTIONS
    st.sidebar.markdown("---")
    st.sidebar.markdown('<h3 style="color: #009999;">🛠️ Actions</h3>', unsafe_allow_html=True)
    
    if st.sidebar.button("📊 Export to Excel", use_container_width=True):
        # Create downloadable Excel file
        if not df.empty:
            df.to_excel("siemens_capacity_plan.xlsx", index=False)
            st.sidebar.success("✅ Excel file exported as 'siemens_capacity_plan.xlsx'")
        else:
    ...
    # SIDEBAR STATS
    st.sidebar.markdown("---")
    st.sidebar.markdown('<h3 style="color: #009999;">📈 Quick Stats</h3>', unsafe_allow_html=True)
    
    if not df.empty:
        st.sidebar.metric("Total Team", len(df))
        st.sidebar.metric("At Risk", len(df[df['days_until_exit'] < 180]))
        st.sidebar.metric("Avg Exit Days", f"{int(df['days_until_exit'].mean())}d")
    else:
